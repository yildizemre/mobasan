                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if(names[c]) =="fire":
                            ...
                            time1=datetime.now()
                            fire_dataframe_array.append([time1,1])
                            fire_dataframe=pd.DataFrame(fire_dataframe_array,columns=['time','value'])

                            lenght_dataframe=len(fire_dataframe)

                            time2=(fire_dataframe.loc[0][0])

                            # print(time2)
                            record=(time1-time2)
                            time_split = divmod(record.total_seconds(), 60)     
                            time_second=time_split[1]
                            # if time_second>10:
                            #     print("TELEGRAM POST")
                            #     annotator.box_label(xyxy, label, color=colors(c, True))
                            #     im_sender = annotator.result()
                            #     sender_image_path = "./sender_images/"+str(time1)+".jpg"

                            #     cv2.imwrite("./sender_images/"+str(time1)+".jpg",im_sender)


                            #     fire_dataframe_array.clear()
                            #     send_message(sender_image_path)


                                
                                # user_details = "@aligokkaya"
                                ...
                                #     #Send the message
                                #     client.send_message(user_details, message_content)
                                #     # /home/emreyildiz/fire/yolov5/sender_images/2022-03-07 23:07:28.927934.jpg
                                #     client.send_file("@aligokkaya","./sender_images/"+str(time1)+".jpg")
                                #     client.disconnect()
